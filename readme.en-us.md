...
> If you're using HTTP services like Nginx and need to automatically issue and install HTTPS certificates, the recommended repo is: [acme.sh](https://github.com/acmesh-official/acme.sh/wiki).
>
...
| get_ipv6_local         | Since v2.2.4, default true. Enable it to get the local ipv6 address from the Python socket module without relying on external, disable it to use HTTP to request other servers to get the local ipv6. |
