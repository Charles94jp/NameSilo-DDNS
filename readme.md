...
|email_after_reboot|Prerequisites: Linux; Set email configuration; Set to start with Linux; Not in docker. Default false. When set to true, if the system `uptime` is more than 4 minutes from the last shutdown time, and the DDNS start time is less than 4 minutes from the system `uptime`, you will receive an email alert to get your current IP.<br> It is applicable to the case of unexpected power failure at home, when the power is turned on, the router will redial and usually get a new IP, if your server supports power on automatically, then DDNS will send you an email to tell you that your server has been started successfully after it is starts with Linux.|
